...
class InvoiceService {
  ...
  async createInvoiceFromVisit(
    ...
    const invoice: Invoice = {
      ...
      clinicDetails: {
        ...
        registrationNumber: 'TBD', // Would be fetched from settings
      },
      lineItems,
      subtotal,
      taxRate,
    ...
    try {
      ...
      console.warn('Failed to save invoice to database:', err);
  ...
  async getPatientInvoices(patientId: string): Promise<Invoice[]> {
    try {
      ...
      return data.map((inv) => this.mapDbInvoice(inv));
    } catch (err) {
  ...
  async getInvoiceSummary(
    ...
    try {
      ...
      return {
        totalIssued: data.reduce((sum, inv) => sum + (inv.total || 0), 0),
        totalPaid: data.reduce((sum, inv) => sum + (inv.amount_paid || 0), 0),
