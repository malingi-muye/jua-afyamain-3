...
export const teamService = {
  ...
  async getTeamMembers(): Promise<TeamMember[]> {
    try {
      ...
      if (!authUser) {
        console.warn('[teamService] No authenticated user');
        return [];
      }
      ...
      if (!clinicId) {
        console.warn('[teamService] User has no clinic_id, returning only current user');
      ...
      if (!users || users.length === 0) {
        console.warn('[teamService] No team members found');
        return [];
      }
      ...
      console.log(`[teamService] Loaded ${teamMembers.length} team members`);
      return teamMembers;
  ...
  async inviteTeamMember(email: string, role: string, invitedBy: string): Promise<InvitationRequest> {
    try {
      ...
      // In a real app, you'd save this to an invitations table
      console.log('[teamService] Simulated invitation:', invitation);
