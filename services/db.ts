...
export const db = {
    ...
    getPatients: async (): Promise<Patient[]> => {
        const { data, error } = await supabase
            .from('patients')
            .select('id, full_name, phone_number, date_of_birth, gender, updated_at, chronic_conditions')
    ...
    createPatient: async (patient: Patient): Promise<Patient> => {
        ...
        const { data, error } = await supabase.from('patients').insert({
            ...
            chronic_conditions: payload.history,
    ...
    getAllClinics: async (): Promise<import('../types').Clinic[]> => {
        ...
        return (data || []).map((c: any) => {
            ...
            const owner = c.users?.find((u: any) => u.role === 'Admin' || u.role === 'SuperAdmin') || c.users?.[0];
            ...
            return {
                ...
                status: (c.status || '').toLowerCase() === 'active' ? 'Active' : (c.status || '').toLowerCase() === 'pending' ? 'Pending' : 'Suspended',
