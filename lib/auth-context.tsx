    ...
    try {
      ...
      if (error) {
        if (error.message?.includes("Refresh Token") || error.message?.includes("Invalid")) {
          console.debug("[Auth Context] Refresh token error, clearing session")
      ...
      if (errorMessage?.includes("Refresh Token") || errorMessage?.includes("Not Found")) {
        console.debug("[Auth Context] Refresh token not found, clearing session and signing out")
        try {
          const supabase = getSupabase()
    ...
    const {
      ...
      try {
        ...
        if (errorMessage?.includes("Refresh Token") || errorMessage?.includes("Invalid")) {
          console.debug("[Auth Context] Auth state change error related to refresh token")
          setUser(null)
          setOrganization(null)
        } else {
