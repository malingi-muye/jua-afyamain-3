...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
    ...
    const handleLoginSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('[Login] Starting signin for:', loginForm.email);
            // Use enterprise auth
            ...
            // Navigate to dashboard - the onAuthStateChange listener will have already fetched user data
    ...
    // Sync user to store when enterprise auth updates (for backward compatibility)
    useEffect(() => {
        if (user && !authLoading) {
            const teamMember: TeamMember = {
                ...
                name: user.fullName,
                email: user.email,
                role: (NEW_ROLE_MAP[user.role] || 'Doctor') as any, // Cast to any to avoid strict union check for now
            ...
            if (onLogin) onLogin(teamMember);
        }
    }, [user, authLoading, actions, onLogin]);
