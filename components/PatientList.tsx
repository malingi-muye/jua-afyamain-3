...
const PatientList: React.FC<PatientListProps> = ({ patients, addPatient, updatePatient, deletePatient, settings }) => {
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
        ...
        const updatedPatient: Patient = {
            ...
            chronicConditions: editFormData.chronicConditions.split(',').map(s => s.trim()).filter(s => s),
    ...
    const handleAddSubmit = (e: React.FormEvent) => {
        ...
        const newPatient: Patient = {
            ...
            chronicConditions: newPatientData.chronicConditions.split(',').map(s => s.trim()).filter(s => s),
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            <span className="text-xs font-bold text-slate-400 uppercase block">Chronic Conditions</span>
                                            <div className="flex flex-wrap gap-2 mt-1">
                                                {patient.chronicConditions && patient.chronicConditions.length > 0 ? (
                                                    patient.chronicConditions.map((c, i) => (
                                                        <span key={i} className="px-2 py-0.5 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded text-xs font-medium border border-orange-100 dark:border-orange-900/30">{c}</span>
                                                    ))
                                                ) : <span className="text-sm text-slate-500 italic">None reported</span>}
                                            </div>
                                        </div>
                                    ...
                                    <div className="pt-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            ...
                                            <div className="col-span-2">
                                                ...
                                                <input
                                                    name="chronicConditions"
                                                    value={newPatientData.chronicConditions}
                                                    onChange={handleAddChange}
                                                    placeholder="e.g. Diabetes, Hypertension"
                                                    className="w-full p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-slate-900 dark:text-white text-sm"
                                                />
