...
const Dashboard: React.FC = () => {
  ...
  const { appointments, patients, inventory, visits, actions, currentUser } = useStore()
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6 mb-5 sm:mb-6 md:mb-8">
        ...
        >
          <div
            className={`p-3 sm:p-3.5 rounded-xl flex-shrink-0 ${inventory.filter((i) => i.stock <= i.minStockLevel).length > 0 ? "bg-red-50 dark:bg-red-900/20" : "bg-teal-50 dark:bg-teal-900/20"}`}
          >
            <Pill
              className={`w-6 h-6 sm:w-7 sm:h-7 ${inventory.filter((i) => i.stock <= i.minStockLevel).length > 0 ? "text-red-600 dark:text-red-400" : "text-teal-600 dark:text-teal-400"}`}
            />
          </div>
          <div className="min-w-0 flex-1">
            ...
            <div className="flex justify-between mb-4 md:mb-6 bg-slate-50 dark:bg-slate-700 p-1.5 sm:p-2 rounded-xl">
              {calendarDays.map((d, index) => (
                <button
                  ...
                  className={`flex flex-col items-center p-1.5 sm:p-2 rounded-lg transition-all text-xs font-bold ${
