      ...
      try {
          switch(action.type) {
              ...
              case 'EDIT_PATIENT':
                  ...
                  const oldValues = { ...pToEdit };
                  const updatedP = { ...pToEdit, ...action.payload.updates };
                  updatePatient(updatedP);
                  // Log audit
                  enterpriseDb.logAudit('patient_updated', 'patient', updatedP.id, oldValues, action.payload.updates);
                  enterpriseDb.logActivity('patient_updated', `Patient updated via WhatsApp`, `Patient: ${updatedP.name}`);
                  return `✅ Updated patient record for ${updatedP.name}`;

              case 'DELETE_PATIENT':
                  const patientToDelete = patients.find(p => p.id === action.payload.patientId);
                  if (patientToDelete) {
                      deletePatient(action.payload.patientId);
                      // Log audit
                      enterpriseDb.logAudit('patient_deleted', 'patient', action.payload.patientId, { name: patientToDelete.name }, {});
                      enterpriseDb.logActivity('patient_deleted', `Patient deleted via WhatsApp`, `Patient: ${patientToDelete.name}`);
                  }
              ...
              case 'ADD_APPOINTMENT':
                  ...
                  // Log audit
                  enterpriseDb.logAudit('appointment_created', 'appointment', newAppt.id, {}, { patientName: newAppt.patientName, date: newAppt.date, time: newAppt.time, source: 'whatsapp' });
                  enterpriseDb.logActivity('appointment_created', `Appointment scheduled via WhatsApp`, `Patient: ${newAppt.patientName}, Date: ${newAppt.date}`);
              ...
              case 'UPDATE_STOCK':
                  ...
                  // Log audit
                  enterpriseDb.logAudit('inventory_updated', 'inventory', item.id, { stock: oldStock }, { stock: newQty });
                  enterpriseDb.logActivity('inventory_updated', `Inventory updated via WhatsApp`, `Item: ${item.name}, Stock: ${oldStock} → ${newQty}`);
              ...
              case 'DELETE_ITEM':
                  ...
                  // Log audit
                  enterpriseDb.logAudit('inventory_deleted', 'inventory', itemToDelete.id, { name: itemToDelete.name, stock: itemToDelete.stock }, {});
                  enterpriseDb.logActivity('inventory_deleted', `Inventory item deleted via WhatsApp`, `Item: ${itemToDelete.name}`);
