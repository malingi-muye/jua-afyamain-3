...
const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({ currentUser, team, activeTab, onLogout }) => {
        ...
        const loadData = async () => {
            if (activeTab === 'overview' || activeTab === 'clinics' || activeTab === 'approvals') {
                try {
                    ...
                    // Convert Pending clinics to requests
                    const clinicRequests = data
                    ...
                    // For now, only show clinic requests. Refund requests can be added later via a separate table
            ...
            // Load system logs from audit_logs (for super admin only)
                ...
                try {
                    ...
                    if (logs) {
                        const systemLogs: SystemLog[] = logs.map((log: any) => ({
                            id: log.id,
                            action: log.action,
                            admin: log.user_name || log.user_email || 'System',
                            target: log.resource_type || log.resource_id || 'Unknown',
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar bg-slate-50/30 dark:bg-transparent">
                                <div className="flex flex-col gap-6">
                                        ...
                                        <div className="space-y-2">
                                            <div className="p-6 bg-white dark:bg-slate-700 rounded-3xl rounded-tl-none border border-slate-100 dark:border-slate-600 shadow-sm">
                                                <p className="text-sm text-slate-700 dark:text-slate-200 font-medium leading-relaxed">
                                                    Hello support team. We've been having issues with the bulk SMS delivery report for the past 24 hours. The messages are being sent, but the delivery stats are staying at "Processing".
                                                </p>
                                            </div>
                                    ...
                                    {/* Admin Message */}
                                    <div className="flex gap-4 max-w-[80%] self-end flex-row-reverse text-right">
                                        <div className="w-10 h-10 rounded-2xl bg-indigo-600 shrink-0 flex items-center justify-center text-white font-black text-xs">SA</div>
                                        <div className="space-y-2 text-right items-end flex flex-col">
                                            <div className="p-6 bg-indigo-600 rounded-3xl rounded-tr-none text-white shadow-xl shadow-indigo-200 dark:shadow-none">
                                                <p className="text-sm font-medium leading-relaxed">
                                                    Hello! We've identified a delay in the upstream SMS provider's callback service. We are implementing a polling fallback to update your reports. This should be resolved within the hour.
                                                </p>
                                            </div>
                                            <span className="text-[10px] font-black text-slate-400 uppercase mr-2">Last Update: {selectedTicket.lastUpdate}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                ...
                                                {actionMenuOpenId === clinic.id && (
                                                    <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                                        <div className="p-2 space-y-1">
                                                            ...
                                                            <button onClick={() => { handleSuspendClinic(clinic.id); setActionMenuOpenId(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition-colors">
                                                            ...
                                                            <button onClick={() => { handleDeleteClinic(clinic.id); setActionMenuOpenId(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-colors">
                                ...
                                <div className="flex-1 overflow-y-auto p-10 space-y-12 no-scrollbar">
                                    ...
                                    <section>
                                        ...
                                        <div className="space-y-4">
                                            {[
                                                { date: '2023-11-15', amount: 5000, status: 'Success', method: 'M-Pesa' },
                                                { date: '2023-10-15', amount: 5000, status: 'Success', method: 'M-Pesa' },
                                                { date: '2023-09-15', amount: 5000, status: 'Success', method: 'Bank Transfer' }
                                            ].map((pay, i) => (
                                                <div key={i} className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-900 rounded-[1.5rem] border border-slate-100 dark:border-slate-700">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <p className="text-[10px] font-black text-green-600 uppercase mt-0.5">{pay.status}</p>
                                                    </div>
                                                </div>
                                            ))}
                                            <button className="w-full py-4 text-xs font-black text-indigo-600 uppercase tracking-widest border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[1.5rem] hover:bg-slate-50 transition-all">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1 rounded-xl w-fit">
                    {['transactions', 'gateways'].map(tab => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all capitalize ${paymentsSubTab === tab ? 'bg-white dark:bg-slate-700 shadow text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'
                                }`}
                        >
                            {tab}
                        </button>
