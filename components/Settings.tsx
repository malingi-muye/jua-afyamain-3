...
const Settings: React.FC<SettingsProps> = ({ isDarkMode, toggleTheme, settings, updateSettings, showToast }) => {
        ...
        try {
            ...
            for (const patient of offlineData.patients) {
                try {
                    ...
                    console.warn('Failed to sync patient:', e);
                    errorCount++;
                }
            }
            ...
            for (const appt of offlineData.appointments) {
                try {
                    ...
                    console.warn('Failed to sync appointment:', e);
                    errorCount++;
                }
            }
    ...
    const processPayment = async () => {
        ...
        try {
            // 1. Configure Service with current form data
            paymentService.initialize(formData.paymentConfig as any);
            ...
            if (formData.paymentConfig.provider === 'M-Pesa') {
                setProcessingStatus('Sending STK Push to your phone...');
                // Use user's phone or clinic phone
                const phone = formData.phone || '254700000000';
                result = await paymentService.initializeMpesaPayment(
                    phone,
                    checkoutItem.amount,
                    'SUBSCRIPTION',
                    checkoutItem.name,
                ...
                setProcessingStatus(`STK Sent! Check your phone (${phone})...`);

                // Simulate waiting for callback (Real app would use websocket/polling)
                await new Promise(r => setTimeout(r, 4000));

            } else if (formData.paymentConfig.provider === 'PayStack') {
                setProcessingStatus('Preparing secure checkout...');
                result = await paymentService.initializePayStackPayment(
                    checkoutItem.amount,
                    formData.email,
                    { plan: checkoutItem.name, type: checkoutItem.type }
                );
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700 text-sm">
                        {formData.team.map((member) => (
                            <tr key={member.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded-lg text-xs font-bold ${member.role === 'Admin' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' :
                ...
                <div className="lg:col-span-3 space-y-2 sticky top-24">
                    {[
                        ...
                        <button
                            ...
                            className={`w-full text-left px-4 py-3.5 font-medium rounded-xl flex items-center justify-between transition-all duration-200 ${activeTab === item.id
