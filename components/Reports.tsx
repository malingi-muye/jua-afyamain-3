...
const Reports: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const { data: visitsWithDoctors } = await supabase
          .from("visits")
          .select("*, users!visits_doctor_id_fkey(id, full_name)")
          .neq("stage", "completed")
        ...
        const mappedVisits = (visitsWithDoctors || []).map((v: any) => ({
          ...
          doctorName: v.users?.full_name || "Unknown Doctor",
          stage: v.stage,
    ...
    loadData()
  }, [timeRange]) // We could optimize to only fetch once, but for time-range specific queries in future, keeping dependency
  ...
  const paymentMethodData = useMemo(() => {
    // Based on visit payment status or mocked method since we don't explicitly store payment method string in visit root
    // We'll mimic this distribution for now
    const mpesaCount = filteredVisits.filter((v) => v.totalBill % 2 === 0).length // Random proxy
    const cashCount = filteredVisits.length - mpesaCount
    const total = filteredVisits.length || 1

    return [
      { name: "M-Pesa", value: Math.round((mpesaCount / total) * 100), color: COLORS.mpesa },
  ...
  const tableData = useMemo(() => {
    ...
    return data
  }, [activeTab, filteredVisits, appointments])
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      if (activeTab === "financial") {
        ...
        // Fallback to client-side export for MOH
        const headers = ["Code", "Disease", "New Cases", "Re-Visits", "Referrals"]
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {mohData.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    ...
                    <td className="px-6 py-3 border-r border-slate-100 dark:border-slate-700 font-medium text-slate-900 dark:text-white">
                      {row.disease}
                    </td>
                    <td className="px-6 py-3 border-r border-slate-100 dark:border-slate-700 text-center font-bold text-slate-700 dark:text-slate-300">
                      {row.newCases}
                    </td>
                    <td className="px-6 py-3 border-r border-slate-100 dark:border-slate-700 text-center text-slate-500">
                      {row.reVisits}
                    </td>
