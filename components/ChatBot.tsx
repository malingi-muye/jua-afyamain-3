...
const ChatBot: React.FC = () => {
  ...
  // Check Gemini availability and fetch user role on component mount
  useEffect(() => {
    const initialize = async () => {
      // Check Gemini availability
      logger.log("[ChatBot] Checking Gemini availability...")
      try {
      ...
      // Fetch user role once and cache it
      try {
  ...
  // Render chat even if unavailable, but show error state
  if (isAvailable === null) {
    // Show a loading/skeleton state for the button or nothing?
    // Better to show nothing while checking to avoid jumpiness
    return null
  }
    ...
    <>
      ...
      <div className={`fixed bottom-6 right-4 sm:bottom-10 sm:right-6 md:bottom-10 md:right-10 z-[110] flex flex-col items-end space-y-2 pointer-events-none ${isOpen ? "hidden" : "flex"}`}>
        ...
        <button
          ...
          className={`w-14 h-14 md:w-16 md:h-16 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.2)] hover:scale-110 active:scale-95 transition-all flex items-center justify-center group pointer-events-auto relative overflow-hidden ${isAvailable
            ? "bg-gradient-to-br from-[#063a33] to-[#0a5c50] text-white"
        ...
        >
          ...
          <span className={`absolute top-3 right-3 w-2.5 h-2.5 rounded-full border-2 border-[#063a33] z-20 ${isAvailable ? "bg-green-400" : "bg-red-400"}`}></span>
        </button>
      </div>
            ...
            <div className="flex items-center space-x-3 min-w-0 relative z-10">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center space-x-1.5 opacity-90">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${isAvailable ? "bg-green-400 animate-pulse" : "bg-red-400"}`}></span>
                  <span className="text-xs uppercase font-semibold tracking-wider text-green-50">{isAvailable ? "Online" : "Offline"}</span>
                </div>
              </div>
            </div>
            <button
