  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      try {
        ...
        console.warn("Supabase auth error (expected in demo mode):", error)
      } finally {
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col md:flex-row transition-colors duration-200 font-sans">
        ...
        <main className="flex-1 w-full md:ml-64 lg:ml-72 transition-all duration-300 px-4 sm:px-5 md:px-6 lg:px-8">
          {isDemoMode && (
            <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 px-3 md:px-4 py-2 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 border-b border-amber-200 dark:border-amber-800 animate-in slide-in-from-top">
              <WifiOff className="w-4 h-4 flex-shrink-0" />
              <span className="hidden sm:inline">
                Demo Mode Active: You are viewing local sample data. Database connection is unavailable.
              </span>
              <span className="sm:hidden">Demo Mode - Local Data Only</span>
            </div>
          )}
        ...
        <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-[100] flex flex-col gap-2 no-print max-w-[calc(100%-2rem)] px-2">
          {toasts.map((toast) => (
            <div
              key={toast.id}
              className={`flex items-center gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl shadow-lg border animate-in slide-in-from-bottom-5 fade-in duration-300 text-xs sm:text-sm ${
                toast.type === "success"
                  ? "bg-white dark:bg-slate-800 border-green-200 dark:border-green-900 text-green-700 dark:text-green-400"
                  : toast.type === "error"
                    ? "bg-white dark:bg-slate-800 border-red-200 dark:border-red-900 text-red-700 dark:text-red-400"
